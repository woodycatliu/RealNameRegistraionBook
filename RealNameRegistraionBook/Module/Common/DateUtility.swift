...
class DateUtility {
    ...
    func getDisplayLastestTime(time: Double) -> String {
        ...
        if diffTime >= 172800 {
            ...
            DateUtility.dateFormatter.dateFormat = "yyyy-MM-dd"
            return DateUtility.dateFormatter.string(from: Date(timeIntervalSince1970: time))
        } else if diffTime > 86400 && diffTime < 172800 {
            // 大於等於1天
            return NSLocalizedString("昨天", comment: "")
        } else if diffTime >= 3600 {
            // 大於等於1小
            return NSLocalizedString("\(Int(diffTime/3600)) 小時前", comment: "")
        } else if diffTime > 60 {
            // 最少顯示一分鐘
            return NSLocalizedString("\(Int(diffTime/60)) 分鐘前", comment: "")
        } else if diffTime > 0 {
