...
//  iOS_MomDad
//
//  Created by Woody Liu on 2021/6/2.
//
...
final class WeakDictionary<ProtoclClass> {
    ...
    func addDelegate(delegate: ProtoclClass, identifier: String) {
        lock.lock()
        mapTable.setObject(delegate as AnyObject, forKey: NSString(string: identifier))
        lock.unlock()
    }
